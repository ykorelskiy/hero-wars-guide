/* =========================================================
   Fall of the Veil (Алекто и Эхо) — F2P гайд
...
const CHAPTERS = {
  1: { amuletLvl: 1,  amuletCost: 0,   coins: 10000,  sapph: 1,  name: 'Спуск во тьму',      boss: 'Край рассвета' },
  2: { amuletLvl: 1,  amuletCost: 0,   coins: 30000,  sapph: 3,  name: 'Коридор легенд',     boss: 'Сердце чащи' },
  3: { amuletLvl: 5,  amuletCost: 50,  coins: 50000,  sapph: 5,  name: 'Плато рассвета',     boss: 'Пепельная тропа' },
  4: { amuletLvl: 10, amuletCost: 160, coins: 70000,  sapph: 7,  name: 'Перекрёсток',        boss: 'Голос руин' },
  5: { amuletLvl: 15, amuletCost: 320, coins: 120000, sapph: 17, name: 'Корни и воля',       boss: 'Безмолвный рубеж' },
  6: { amuletLvl: 20, amuletCost: 530, coins: 170000, sapph: 27, name: 'Безумная баллада',   boss: 'Эхо машины' },
  7: { amuletLvl: 25, amuletCost: 790, coins: 220000, sapph: 37, name: 'Волна бед',          boss: 'Шёпот корней' }
};
...
export function simulateAlecto(budget, tomeLvl, grailLvl, chapter) {
...
let isAlectoInitialized = false;

export function initAlectoGuide() {
  if (isAlectoInitialized) return;
  isAlectoInitialized = true;

  initCalculator();
  buildBars('alectoBarsStones', 'stones', false);
  buildBars('alectoBarsSapph', 'sapph', true);
  animateBarsOnView();
  initNav();
  initScrollSpy();

  window.AlectoCalc = {
    simulate: simulateAlecto,
    starsFor,
    CHAPTERS
  };
}
...
function initCalculator() {
  const widget = document.getElementById('alectoCalcWidget');
  if (!widget) return;

  const inputs = {
    budget:  document.getElementById('alectoInpBudget'),
    tome:    document.getElementById('alectoInpTome'),
    grail:   document.getElementById('alectoInpGrail'),
    chapter: document.getElementById('alectoInpChapter')
  };

  const outputs = {
    budget:  document.getElementById('alectoOutBudget'),
    tome:    document.getElementById('alectoOutTome'),
    grail:   document.getElementById('alectoOutGrail'),
    chapter: document.getElementById('alectoOutChapter')
  };

  const results = {
    spent:  document.getElementById('alectoResSpent'),
    left:   document.getElementById('alectoResLeft'),
    coins:  document.getElementById('alectoResCoins'),
    pull:   document.getElementById('alectoResPull'),
    pulls:  document.getElementById('alectoResPulls'),
    stones: document.getElementById('alectoResStones'),
    stars:  document.getElementById('alectoResStars'),
    sapph:  document.getElementById('alectoResSapph')
  };

  const msgBox = document.getElementById('alectoCalcMsg');
  ...
  function render() {
    ...
    const r = simulateAlecto(budget, tome, grail, chapter);
    ...
    if (results.stars) results.stars.textContent = stars > 0 ? ('Алекто ' + stars + '★') : 'меньше 1★';
  ...
  function findBest(budget, chapter) {
    ...
    for (let t = 1; t <= 15; t++) {
      for (let g = 1; g <= 18; g++) {
        const r = simulateAlecto(budget, t, g, chapter);
...
function buildBars(containerId, key, gold) {
  ...
  const data = PLANS.map(p => {
    const r = simulateAlecto(p.budget, p.tome, p.grail, p.chapter);
...
function animateBarsOnView() {
  const fills = document.querySelectorAll('#guide-container-alecto .bar-fill');
...
function initNav() {
  const toggle  = document.getElementById('alectoNavToggle');
  const sidebar = document.getElementById('alectoSidebar');
...
function initScrollSpy() {
  const links = Array.from(document.querySelectorAll('#alectoToc a[href^="#"]'));
